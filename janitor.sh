...
{
	date=$(date)
	timestamp=`cat /var/log/auth.log | grep "New session.*ubuntu" | tail -n2 | head -n1 -c15`
	date --set="$timestamp"
	lastlog -Su ubuntu
	date --set="$date"
	...
	unset tmp timestamp userport userip uphex uihex portoffset ipoffset1 ipoffset2 date
}
